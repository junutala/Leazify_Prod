-- ============================================================
-- PropFlow Force Clear All Seed Data
-- Uses TRUNCATE CASCADE to wipe all tables completely,
-- then re-inserts fresh, consistent test data.
-- ============================================================

-- ============================================================
-- STEP 1: TRUNCATE ALL TABLES (CASCADE handles FK order)
-- ============================================================
DO $$
BEGIN
  -- Truncate all operational and master data tables in one shot
  -- CASCADE automatically handles foreign key dependencies
  TRUNCATE TABLE
    public.work_order_service_requests,
    public.co_tenants,
    public.provider_project_assignments,
    public.estate_assignments,
    public.person_contacts,
    public.turnover_rent,
    public.audit_logs,
    public.bulk_upload_logs,
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Truncate encountered an error: %', SQLERRM;
END $$;

-- Remove seeded user profiles and auth users
DO $$
BEGIN
  ...
  RAISE NOTICE 'User profiles and auth users cleared.';
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'User cleanup error: %', SQLERRM;
END $$;
...
BEGIN
  ...
  UPDATE public.user_profiles SET person_id = person1_id WHERE id = tenant_auth_id;
  ...
  UPDATE public.user_profiles SET provider_id = sp1_id WHERE id = provider_auth_id;
  ...
  VALUES
    ...
    (sr3_id, unit5_id, prop2_id, tenant3_id, admin_auth_id,
     ...
     'Power socket near the checkout counter in R-001 is sparking when devices are plugged in. Safety hazard.',
    ...
    (sr5_id, NULL, prop1_id, NULL, manager_auth_id,
     ...
     'Three ceiling lights in the main lobby of Tower A are out. Common area maintenance required.',
  ...
  RAISE NOTICE 'PropFlow fresh seed data inserted successfully.';
  RAISE NOTICE '---------------------------------------------------';
