      ...
      >
        <div className="relative" style={{ background: 'linear-gradient(135deg,#0d1c2e,#1f3d63)', padding: '32px 32px 28px' }}>
          <button onClick={onClose} className="absolute flex items-center justify-center"
            style={{ top: 18, right: 20, background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', width: 32, height: 32, borderRadius: '50%', fontSize: 16, cursor: 'pointer' }}>✕</button>
          ...
          <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, color: '#fff', marginBottom: 8 }}>{t.lp_modal_title}</h3>
          <p style={{ fontSize: 13, color: '#7fb3d3', lineHeight: 1.55 }}>{t.lp_modal_subtitle}</p>
        </div>
        <div style={{ padding: '30px 32px' }}>
          {!success ? (
            <>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: '#1a2e4a', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>{t.lp_modal_name}</label>
              <input type="text" value={name} onChange={e => setName(e.target.value)}
                placeholder={t.lp_modal_name_placeholder} autoComplete="name"
                style={{ display: 'block', width: '100%', padding: '13px 16px', border: '1.5px solid #d0e4f0', borderRadius: 8, fontSize: 14, color: '#0d1c2e', outline: 'none', marginBottom: 20 }}
              ...
              />
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: '#1a2e4a', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>{t.lp_modal_email}</label>
              <input type="email" value={email} onChange={e => { setEmail(e.target.value); setEmailError(false); }}
                placeholder={t.lp_modal_email_placeholder} autoComplete="email"
                style={{ display: 'block', width: '100%', padding: '13px 16px', border: `1.5px solid ${emailError ? '#c0392b' : '#d0e4f0'}`, borderRadius: 8, fontSize: 14, color: '#0d1c2e', outline: 'none', marginBottom: emailError ? 6 : 20 }}
                onFocus={e => { e.currentTarget.style.borderColor = '#F5A623'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(245,166,35,0.1)'; }}
                onBlur={e => { e.currentTarget.style.borderColor = emailError ? '#c0392b' : '#d0e4f0'; e.currentTarget.style.boxShadow = 'none'; }}
              />
              {emailError && <p style={{ fontSize: 12, color: '#c0392b', marginBottom: 14 }}>{t.lp_modal_email_error}</p>}
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: '#1a2e4a', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>{t.lp_modal_phone}</label>
              <input type="tel" value={phone} onChange={e => { setPhone(e.target.value); setPhoneError(false); }}
                placeholder={t.lp_modal_phone_placeholder} autoComplete="tel"
                style={{ display: 'block', width: '100%', padding: '13px 16px', border: `1.5px solid ${phoneError ? '#c0392b' : '#d0e4f0'}`, borderRadius: 8, fontSize: 14, color: '#0d1c2e', outline: 'none', marginBottom: phoneError ? 6 : 20 }}
                onFocus={e => { e.currentTarget.style.borderColor = '#F5A623'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(245,166,35,0.1)'; }}
                onBlur={e => { e.currentTarget.style.borderColor = phoneError ? '#c0392b' : '#d0e4f0'; e.currentTarget.style.boxShadow = 'none'; }}
              />
              {phoneError && <p style={{ fontSize: 12, color: '#c0392b', marginBottom: 14 }}>{t.lp_modal_phone_error}</p>}
              <button onClick={handleSubmit} disabled={loading}
                style={{ width: '100%', background: loading ? '#b0b8c4' : '#F5A623', color: '#fff', border: 'none', borderRadius: 8, padding: 15, fontSize: 15, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                {loading ? <><Loader2 size={16} className="animate-spin" /> {t.lp_modal_submitting}</> : t.lp_modal_submit}
              </button>
              <p style={{ fontSize: 11, color: '#7fb3d3', textAlign: 'center', marginTop: 12 }}>{t.lp_modal_privacy}</p>
            </>
          ) : (
            <div style={{ textAlign: 'center', padding: '8px 0' }}>
              <div style={{ fontSize: 52, marginBottom: 16 }}>🎉</div>
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, color: '#1a2e4a', marginBottom: 12 }}>{t.lp_modal_success_title}</h3>
              <p style={{ fontSize: 14, color: '#5a6a85', lineHeight: 1.65 }}>{t.lp_modal_success_desc}</p>
              <p style={{ marginTop: 14, fontSize: 13, color: '#5a6a85' }}>
                <a href="https://www.leazify.me" target="_blank" rel="noopener noreferrer" style={{ color: '#F5A623' }}>{t.lp_modal_success_link}</a>
              </p>
            </div>
          )}
        </div>
...
export default function LandingPage() {
  ...
  const challenges = [
    ...
    { icon: '🔧', title: t.lp_prob4_title, desc: t.lp_prob4_desc },
  ];

  const features = [
    { num: '01', title: t.lp_feat1_title, desc: t.lp_feat1_desc },
    { num: '02', title: t.lp_feat2_title, desc: t.lp_feat2_desc },
    { num: '03', title: t.lp_feat3_title, desc: t.lp_feat3_desc },
    { num: '04', title: t.lp_feat4_title, desc: t.lp_feat4_desc },
    { num: '05', title: t.lp_feat5_title, desc: t.lp_feat5_desc },
    { num: '06', title: t.lp_feat6_title, desc: t.lp_feat6_desc },
    { num: '07', title: t.lp_feat7_title, desc: t.lp_feat7_desc },
    { num: '08', title: t.lp_feat8_title, desc: t.lp_feat8_desc },
  ];

  const uspCards = [
    { icon: '👁️', title: t.lp_usp1_title, items: [t.lp_usp1_item1, t.lp_usp1_item2, t.lp_usp1_item3] },
    { icon: '📝', title: t.lp_usp2_title, items: [t.lp_usp2_item1, t.lp_usp2_item2, t.lp_usp2_item3] },
  ];

  const outcomes = [
    { icon: '📈', label: t.lp_outcome1 },
    { icon: '🏠', label: t.lp_outcome2 },
    { icon: '😊', label: t.lp_outcome3 },
    { icon: '💰', label: t.lp_outcome4 },
    { icon: '📊', label: t.lp_outcome5 },
  ];

  const trustItems = [
    { icon: '🔒', label: t.lp_trust_security },
    { icon: '👤', label: t.lp_trust_roles },
    { icon: '☁️', label: t.lp_trust_cloud },
    { icon: '💾', label: t.lp_trust_backups },
    { icon: '⚡', label: t.lp_trust_dashboards },
  ];

  // RTL-aware float card position
  const floatCardStyle = isRTL
    ? { position: 'absolute' as const, left: -28, top: -28, bottom: 'auto', background: 'var(--lp-orange)', borderRadius: 12, padding: '14px 20px', boxShadow: '0 16px 40px rgba(244,130,31,0.4)', display: 'flex', alignItems: 'center', gap: 12 }
    : { position: 'absolute' as const, right: -28, top: -28, bottom: 'auto', background: 'var(--lp-orange)', borderRadius: 12, padding: '14px 20px', boxShadow: '0 16px 40px rgba(244,130,31,0.4)', display: 'flex', alignItems: 'center', gap: 12 };
  ...
  return (
    <>
      <style>{`
        ...
        /* RTL adjustments */
        [dir="rtl"] .lp-challenge-card{border-left:none;border-right:4px solid var(--lp-orange);}
      `}</style>
      ...
      <div className="lp-root">
        ...
        <nav ref={navRef} style={{
          ...
          <div className={`lp-nav-links${navOpen ? ' open' : ''}`} style={{ display: 'flex', alignItems: 'center', gap: 28 }}>
            <a href="#problems" className="lp-nav-link" onClick={() => setNavOpen(false)}>{t.lp_nav_challenges}</a>
            <a href="#features" className="lp-nav-link" onClick={() => setNavOpen(false)}>{t.lp_nav_features}</a>
            <a href="#usp" className="lp-nav-link" onClick={() => setNavOpen(false)}>{t.lp_nav_why}</a>
            <a href="#outcomes" className="lp-nav-link" onClick={() => setNavOpen(false)}>{t.lp_nav_outcomes}</a>
            <a href="#contact" className="lp-nav-link" onClick={() => setNavOpen(false)}>{t.lp_nav_contact}</a>
            {/* Language toggle — prominent position between nav links and action buttons */}
            <LangToggle />
            <button className="lp-btn-ghost" onClick={() => { router.push('/sign-up-login-screen'); setNavOpen(false); }}>{t.lp_nav_signin}</button>
            <button className="lp-btn-blue" onClick={() => { setShowDemo(true); setNavOpen(false); }}>{t.lp_nav_demo}</button>
          </div>

          {/* Hamburger */}
          <div className="lp-hamburger" style={{ display: 'none', flexDirection: 'column', gap: 5, cursor: 'pointer', padding: 4 }} onClick={() => setNavOpen(o => !o)}>
        ...
        <section style={{
          ...
          <div className="lp-hero-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center', maxWidth: 1280, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
            <div className="lp-hero-content" style={{ order: isRTL ? 2 : 1 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 20, padding: '6px 14px', marginBottom: 28 }}>
                <span className="lp-badge-dot" style={{ width: 6, height: 6, borderRadius: '50%', background: '#F5A623', display: 'inline-block' }} />
                <span style={{ fontSize: 11, fontWeight: 600, color: '#fff', letterSpacing: 2, textTransform: 'uppercase' }}>{t.lp_badge}</span>
              </div>
              ...
              <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(22px,2.8vw,46px)', lineHeight: 1.25, color: '#fff', marginBottom: 24, letterSpacing: -1 }}>
                <span style={{ display: 'block', color: '#fff' }}>{t.lp_hero_title1}</span>
                <span style={{ display: 'block', color: '#fff' }}>{t.lp_hero_title2}</span>
                <span style={{ display: 'block', color: '#f4821f', textDecoration: 'underline' }}>{t.lp_hero_title3}</span>
              </h1>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 18, flexWrap: 'wrap' }}>
                <button className="lp-btn-primary" onClick={() => setShowDemo(true)}>{t.lp_hero_cta_demo}</button>
                ...
                >{t.lp_hero_cta_features}</a>
              </div>
            </div>

            <div className="lp-hero-visual" style={{ position: 'relative', order: isRTL ? 1 : 2 }}>
              <div style={{ position: 'relative' }}>
                <img src="/assets/images/dashboard_screenshot-1779424600836.jpg" alt="Leazify Dashboard" className="lp-screenshot" />
                <div style={floatCardStyle}>
                  <span style={{ fontSize: 22 }}>📈</span>
                  <div style={{ color: '#fff' }}>
                    <strong style={{ display: 'block', fontSize: 14, fontWeight: 600 }}>{t.lp_hero_float_title}</strong>
                    <span style={{ fontSize: 11, opacity: 0.8 }}>{t.lp_hero_float_sub}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section id="problems" style={{ padding: '96px 6vw' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: '#F5A623', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 14 }}>{t.lp_problems_badge}</p>
        ...
        <section id="features" style={{ background: '#1a2e4a', padding: '96px 6vw', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ maxWidth: 1280, margin: '0 auto', position: 'relative', zIndex: 1 }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: '#F5A623', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 14 }}>{t.lp_features_badge}</p>
        ...
        <section id="usp" style={{ background: '#fff9f2', padding: '96px 6vw', borderTop: '1px solid #f4e4d0' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: '#F5A623', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 14 }}>{t.lp_usp_badge}</p>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(30px,3.5vw,48px)', color: '#1a2e4a', lineHeight: 1.2, letterSpacing: -1, marginBottom: 16 }}>
              {t.lp_usp_title}
            </h2>
                ...
                <div key={card.title} className="lp-usp-card lp-animate-card">
                  ...
                  <div>
                    ...
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {card.items.map(item => (
                        <li key={item} style={{ fontSize: 14, color: '#5a6a85', paddingInlineStart: 18, position: 'relative', lineHeight: 1.55 }}>
                          <span style={{ position: 'absolute', insetInlineStart: 0, color: '#F5A623', fontWeight: 700, fontSize: 12, top: 1 }}>✓</span>
                          {item}
                        </li>
                      ))}
                    </ul>
        ...
        <section id="outcomes" style={{ background: '#fff', padding: '96px 6vw' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: '#F5A623', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 14 }}>{t.lp_outcomes_badge}</p>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(30px,3.5vw,48px)', color: '#1a2e4a', lineHeight: 1.2, letterSpacing: -1, marginBottom: 16 }}>
              {t.lp_outcomes_title}
            </h2>
        ...
        <section style={{
          ...
          <div style={{ position: 'relative', zIndex: 1, maxWidth: 640, margin: '0 auto' }}>
            ...
            <button className="lp-btn-dark" onClick={() => setShowDemo(true)}>{t.lp_cta_btn}</button>
          </div>
        </section>

        {/* ── FOOTER ── */}
        <footer id="contact" style={{ background: '#0d1c2e', padding: '56px 6vw 28px' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            <div className="lp-footer-top" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 40, marginBottom: 48, flexWrap: 'wrap' }}>
              <div>
                ...
                <p style={{ fontSize: 13, color: '#7fb3d3', lineHeight: 1.7, maxWidth: 260 }}>{t.lp_footer_tagline}</p>
              </div>
              <div className="lp-footer-links" style={{ display: 'flex', gap: 64, flexWrap: 'wrap' }}>
                <div>
                  <h4 style={{ fontSize: 11, fontWeight: 600, color: '#F5A623', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 18 }}>{t.lp_footer_product}</h4>
                  {[
                    { label: t.lp_nav_features, href: '#features' },
                    { label: t.lp_nav_why, href: '#usp' },
                    { label: t.lp_nav_outcomes, href: '#outcomes' },
                  ].map(l => (
                ...
                <div>
                  <h4 style={{ fontSize: 11, fontWeight: 600, color: '#F5A623', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 18 }}>{t.lp_footer_contact}</h4>
                  <a href="https://www.leazify.me" target="_blank" rel="noopener noreferrer" style={{ display: 'block', fontSize: 13, color: '#7fb3d3', textDecoration: 'none', marginBottom: 10, transition: 'color 0.2s' }}
                    ...
                    onMouseLeave={e => (e.currentTarget.style.color = '#7fb3d3')}
                  >www.leazify.me</a>
                  <a href="mailto:contact@leazify.me" style={{ display: 'block', fontSize: 13, color: '#7fb3d3', textDecoration: 'none', marginBottom: 10, transition: 'color 0.2s' }}
                    onMouseEnter={e => (e.currentTarget.style.color = '#fff')}
                    onMouseLeave={e => (e.currentTarget.style.color = '#7fb3d3')}
                  >contact@leazify.me</a>
                </div>
              </div>
            </div>
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              <p style={{ fontSize: 12, color: '#3d5068' }}>© {currentYear} {t.lp_footer_copyright}</p>
              <p style={{ fontSize: 12, color: '#3d5068' }}>
                <a href="#" style={{ color: '#3d5068', textDecoration: 'none' }}>{t.lp_footer_privacy}</a>
                {' · '}
                <a href="#" style={{ color: '#3d5068', textDecoration: 'none' }}>{t.lp_footer_terms}</a>
              </p>
            </div>
          </div>
        </footer>
